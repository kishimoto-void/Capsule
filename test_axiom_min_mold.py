...
class TestAxiomMinMold(unittest.TestCase):
    def setUp(self):
        self.inner = Inner(
            alpha=Alpha(),
            beta=Beta(
            ...
            facts=(
                BetaFact("K-1", "sys", "AXIOM"),
                BetaFact("K-2", "env", "prod"),
            ),
        )
    ...
    def test_inner_integrity_same_object(self):
        """seal → hash固定 → 同じInnerの中身を差し替え → intact() is False"""
        self.assertTrue(self.cap.inner.intact())
        frozen = self.cap.inner.hash_a

        self.cap.inner.beta = Beta(
            name="改ざん体",
            tone="簡潔",
            center="中心軸",
            values=("不変",),
    ...
    def test_inner_distinct_payload_hash(self):
        other = Inner(
            alpha=Alpha(),
            beta=Beta(name="改ざん体", tone="簡潔", center="中心軸", values=("不変",)),
            facts=self.inner.facts,
        ).seal()
        self.assertNotEqual(self.cap.inner.hash_a, other.hash_a)
    ...
    def test_render_bind_switch(self):
        filt = {"project": "AXIOM"}
        rendered_normal = self.cap.render("質問", filt)
        self.assertIn("βに従って短く", rendered_normal)
        self.cap.eta.step(tone=0.1, identity=0.1, values=0.1)
        rendered_high = self.cap.render("質問", filt)
        self.assertIn("偏差が大きい。βへ戻せ", rendered_high)
